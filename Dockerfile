...
ENV FIREFOX_VERSION="${FF_VER}" \
  ...
  MULTINODE="false" \
  FFMPEG_FRAME_RATE=4 \
  FFMPEG_CODEC_ARGS="-vf scale=1280:-1 -vcodec libx264 -preset ultrafast -pix_fmt yuv420p" \
  FFMPEG_CODEC_VA_ARGS="-vf scale=1280:-1 -vcodec libx264 -acodec copy -preset ultrafast" \
  FFMPEG_FINAL_CRF=0 \
  FFMPEG_DRAW_MOUSE=1 \
  ...
  GA_TRACKING_ID="UA-11395938581854-9" \
